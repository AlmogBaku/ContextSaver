...
| `prompt.submit` | — | If `e.origin?.kind === 'plugin'` or the text starts with `/saver` → `next(e)` (the command path runs its own check). `extra = [...state.notes, ...state.standing]`; non-empty → `dispatch(notes.drained)` and `carried = { ...e, context: [...(e.context ?? []), ...extra] }`, else `carried = e`. Then, before returning, `armedCheck()`: the person's next prompt is an opportunity too, so a reload and one line of typing audit the session at once. **No cadence lane here** — a prompt is not new work, only the arming justifies a run — and the fire-and-forget run never changes what the prompt carries. `return next(carried)`. |
